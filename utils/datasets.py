...
class reloadFrame:
    def __init__(self,source='streams.txt', img_size=640, stride=32):
    ...
    def __next__(self):
        if self.load == True and self.task > 0:
            ...
            if ret_value==ord('a') or ret_value==ord('A'):
                self.load = not self.load
            #self.cap.release()
            self.task -= 1
            return self.source, img, img0, None
        elif self.load == False and self.task > 0:
...
class GetAShot: # for inference
    ...
    def __next__(self):
        ...
        if self.cap.isOpened():
            ret, img0 = self.cap.read()
            self.count += 1
            print("type: img0 {}".format(img0.shape))
            if ret:
                # Padded resize
                img = letterbox(img0, self.img_size, stride=self.stride)[0]
